...
from common import TYPE_INIT, TYPE_AGREE, TYPE_REQUEST, TYPE_ANSWER
...
def main() -> None:
    ...
    parser.add_argument("--file_path", default="sample.txt", help="待发送的 ASCII 文本文件")
    parser.add_argument("--lmin", type=int, default=50, help="每块最小字节数")
    parser.add_argument("--lmax", type=int, default=100, help="每块最大字节数")
    parser.add_argument("--chunk_seed", type=int, default=42, help="随机分块种子（用于复现）")
    ...
    # 1) 发送 Initialization
    init_pkt = struct.pack("!BI", TYPE_INIT, n_blocks)
    ...
    for i, sz in enumerate(chunk_sizes):
        ...
        req = struct.pack("!BI", TYPE_REQUEST, sz) + chunk_data
        sock.sendall(req)
